...
def resolve_python_spec() -> str:
    ...
    if not candidate.exists():
        print(f"[ERROR] Version file not found: {candidate}")
        sys.exit(1)

    spec = candidate.read_text(encoding="utf-8").strip()
    if not spec:
        print(f"[ERROR] Version file is empty: {candidate}")
        sys.exit(1)
    return spec
...
def ensure_uv_python(spec: str) -> Path:
    ...
    print(f"[INFO] Ensuring uv Python {spec} is installed.")
    try:
    ...
    if not path.exists():
        print(f"[ERROR] Located uv python path does not exist: {path}")
        sys.exit(1)
    return path
...
def ensure_uv_venv(spec: str) -> None:
    ...
    print(f"[INFO] Creating uv virtual environment at {VENV_DIR}.")
    try:
        subprocess.run(
            ["uv", "venv", "--python", spec],
            check=True,
    ...
    if not interpreter.exists():
        print(f"[ERROR] Virtual environment interpreter missing: {interpreter}")
        sys.exit(1)
...
def existing_requirement_files() -> list[Path]:
    ...
    if platform_file and platform_file.exists():
        ...
        print(f"[INFO] Platform-specific requirements file missing: {platform_file}")
