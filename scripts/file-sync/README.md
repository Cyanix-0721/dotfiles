...
python file_sync.py -p 1

# 模拟运行（不实际执行）
# Dry run (do not execute)
python file_sync.py -p 1 --dry-run

# 详细输出
# Verbose output
python file_sync.py -p 1 --verbose

# 增量更新模式（不删除目标中的文件）
# Update mode (do not delete files in destination)
python file_sync.py -p 1 --mode update

# 不排除空文件夹
# Do not exclude empty directories
python file_sync.py -p 1 --no-exclude-empty

# 自动创建目标目录
# Auto-create destination directory
python file_sync.py -p 1 --auto-create-dest

# 组合使用
# Combined usage
python file_sync.py -p 1 --mode mirror --dry-run --verbose
```
...
```bash
python file_sync.py -p 1 --mode mirror --dry-run
```
...
python file_sync.py -p 1 --mode safe
```
